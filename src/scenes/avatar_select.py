...
class AvatarSelectScene(Scene):
    ...
    def startup(self, persistent):
        ...
        max_sprite_w = card_w - 16   # 8px padding each side
        max_sprite_h = card_h - 40   # room for label at bottom
        self.rects = []
        for i, species in enumerate(SPECIES):
            path = SPRITE_DIR / "characters" / f"{species}_base.png"
            if path.exists():
            ...
            self.rects.append(pygame.Rect(start_x + i * (card_w + spacing), 110, card_w, card_h))

        self.confirm_rect = pygame.Rect(0, 0, 180, 38)
        self.confirm_rect.center = (LOGICAL_WIDTH // 2, 370)
    ...
    def draw(self, surface):
        ...
        surface.blit(header, (40, 20))
        pygame.draw.line(surface, COLOR_RULE_LINE, (40, 38), (LOGICAL_WIDTH - 40, 38), 1)

        title = self.title_font.render("Choose Your Avatar", True, COLOR_ACCENT_DARK)
        surface.blit(title, title.get_rect(center=(LOGICAL_WIDTH // 2, 65)))
        sub = self.small_font.render("Your representative for the admissions process.", True, COLOR_TEXT_LIGHT)
        surface.blit(sub, sub.get_rect(center=(LOGICAL_WIDTH // 2, 90)))
        ...
        for i, species in enumerate(SPECIES):
            ...
            pygame.draw.rect(surface, border, rect, 2 if is_sel else 1, border_radius=6)
            sprite = self.sprites[species]
            # Center sprite horizontally and vertically in the card (above label)
            sprite_area_h = rect.height - 36  # leave room for label
            sx = rect.centerx - sprite.get_width() // 2
            sy = rect.y + 6 + (sprite_area_h - sprite.get_height()) // 2
            ...
            surface.blit(label, label.get_rect(center=(rect.centerx, rect.bottom - 16)))

        # Confirm button
        pygame.draw.rect(surface, COLOR_BUTTON_IDLE, self.confirm_rect, border_radius=4)
        pygame.draw.rect(surface, COLOR_PANEL_BORDER, self.confirm_rect, 1, border_radius=4)
        ...
        surface.blit(hint, hint.get_rect(center=(LOGICAL_WIDTH // 2, LOGICAL_HEIGHT - 50)))
