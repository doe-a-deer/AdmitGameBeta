...
class DecisionScene(Scene):
    ...
    def startup(self, persistent):
        ...
        self.state = "envelope"
        self.fade_timer = 0
        self.font = pygame.font.SysFont("Georgia", 13, bold=True)
        ...
        self.envelope_rect = pygame.Rect(0, 0, 260, 150)
        self.envelope_rect.center = (LOGICAL_WIDTH//2, LOGICAL_HEIGHT//2 - 10)
        self.continue_rect = pygame.Rect(0, 0, 160, 34)
        self.continue_rect.center = (LOGICAL_WIDTH//2, LOGICAL_HEIGHT - 60)
    ...
    def _draw_envelope(self, surface):
        ...
        surface.blit(t, t.get_rect(center=(cx, 65)))

        er = self.envelope_rect
        pygame.draw.rect(surface, (235, 220, 195), er, border_radius=4)
        pygame.draw.rect(surface, COLOR_ACCENT, er, 2, border_radius=4)
        flap = [(er.left+2, er.top+2), (er.centerx, er.centery-20), (er.right-2, er.top+2)]
        pygame.draw.polygon(surface, (225, 210, 185), flap)
        pygame.draw.polygon(surface, COLOR_ACCENT, flap, 1)
        pygame.draw.circle(surface, COLOR_ACCENT, (er.centerx, er.centery-20), 14)
        pygame.draw.circle(surface, COLOR_ACCENT_DARK, (er.centerx, er.centery-20), 14, 2)
        seal = self.tiny_font.render("H", True, COLOR_BG)
        surface.blit(seal, (er.centerx - seal.get_width()//2, er.centery-20-seal.get_height()//2))
        ns = self.font.render(name, True, COLOR_TEXT)
        surface.blit(ns, (er.centerx - ns.get_width()//2, er.centery+15))
        h = self.small_font.render("Click to open", True, COLOR_TEXT_LIGHT)
        surface.blit(h, (cx - h.get_width()//2, er.bottom+20))
    ...
    def _draw_revealed(self, surface):
        ...
        surface.blit(self.small_font.render(name, True, COLOR_TEXT_DIM), (cx - 80, 25))
        pygame.draw.line(surface, COLOR_RULE_LINE, (80, 42), (LOGICAL_WIDTH-80, 42), 1)
        rt = self.title_font.render(RESULT_LABELS[result], True, rc)
        surface.blit(rt, rt.get_rect(center=(cx, 65)))
        ...
        y = 105
        for line in self._wrap(letter, self.small_font, LOGICAL_WIDTH-120):
            surface.blit(self.small_font.render(line, True, COLOR_TEXT), (60, y)); y += 18
    ...
    def _draw_summary(self, surface):
        ...
        surface.blit(t, t.get_rect(center=(cx, 45)))
        pygame.draw.line(surface, COLOR_RULE_LINE, (100, 70), (LOGICAL_WIDTH-100, 70), 1)
        y = 90
        for name, result in self.decisions.items():
            surface.blit(self.font.render(name, True, COLOR_TEXT), (80, y))
            rs = self.font.render(RESULT_LABELS[result], True, RESULT_COLORS[result])
            surface.blit(rs, (LOGICAL_WIDTH-80-rs.get_width(), y))
            pygame.draw.line(surface, COLOR_RULE_LINE, (80, y+22), (LOGICAL_WIDTH-80, y+22), 1)
            y += 40
        f = self.tiny_font.render("All decisions reflect our commitment to institutional excellence.", True, COLOR_TEXT_LIGHT)
        surface.blit(f, (cx - f.get_width()//2, y+20))
        self.continue_rect.center = (cx, LOGICAL_HEIGHT-60)
