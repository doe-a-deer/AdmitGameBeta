...
class PersonalityTestScene(Scene):
    ...
    def startup(self, persistent):
        ...
        self.typewriter_timer = 0
        self.typewriter_index = 0
        self.font = pygame.font.SysFont("Georgia", 13)
        self.small_font = pygame.font.SysFont("Georgia", 11)
        self.q_font = pygame.font.SysFont("Georgia", 15, italic=True)
    ...
    def draw(self, surface):
        ...
        surface.blit(header, (40, 25))
        pygame.draw.line(surface, COLOR_RULE_LINE, (40, 44), (LOGICAL_WIDTH - 40, 44), 1)
    ...
    def _draw_question(self, surface):
        ...
        lines = self._wrap_text(visible, self.q_font, LOGICAL_WIDTH - 100)
        y = 65
        for line in lines:
            surface.blit(self.q_font.render(line, True, COLOR_ACCENT_DARK), (50, y))
            y += 22

        y = max(y + 25, 140)
        self.answer_rects = []
        for i, answer in enumerate(question["answers"]):
            rect = pygame.Rect(50, y, LOGICAL_WIDTH - 100, 50)
            ...
            bx, by = rect.x + 18, rect.centery
            if is_sel:
                pygame.draw.circle(surface, COLOR_ACCENT, (bx, by), 5)
            else:
                pygame.draw.circle(surface, COLOR_PANEL_BORDER, (bx, by), 5, 1)
            alines = self._wrap_text(answer["text"], self.font, rect.width - 50)
            ay = rect.y + 8
            for al in alines:
                color = COLOR_TEXT if is_sel else COLOR_TEXT_DIM
                surface.blit(self.font.render(al, True, color), (rect.x + 36, ay))
                ay += 18
            y += 60

        hint = self.small_font.render(
            "\u2191\u2193 or mouse  \u00b7  Enter to confirm", True, COLOR_TEXT_LIGHT)
        surface.blit(hint, (50, LOGICAL_HEIGHT - 55))

    def _draw_processing(self, surface):
        cx = LOGICAL_WIDTH // 2
        label = self.q_font.render("Processing your profile\u2026", True, COLOR_TEXT)
        surface.blit(label, (cx - label.get_width() // 2, LOGICAL_HEIGHT // 2 - 40))
        bar_w, bar_h = 260, 14
        ...
        pygame.draw.rect(surface, COLOR_PANEL_BORDER, (bar_x, bar_y, bar_w, bar_h), border_radius=7)
        fw = int(bar_w * progress)
        if fw > 0:
            pygame.draw.rect(surface, COLOR_ACCENT, (bar_x, bar_y, fw, bar_h), border_radius=7)
        sub = self.small_font.render("Holistic deliberation in progress\u2026", True, COLOR_TEXT_LIGHT)
        surface.blit(sub, (cx - sub.get_width() // 2, bar_y + 24))
